...
class PromptBuilderImpl @Inject constructor() : PromptBuilder {
    ...
    override fun build(context: RetrievalContext, userQuery: String): String {
        ...
        appendLine("  \"projectId\": \"<projectId from active projects list if task belongs to that project, else omit>\"")
        appendLine("}")
        appendLine("[/LIFEPILOT_ACTION]")
        appendLine("IMPORTANT: If the user says 'add a task to [project name]' or 'add this to my [project] project', you MUST include the matching projectId from the active projects list above.")
        appendLine()
